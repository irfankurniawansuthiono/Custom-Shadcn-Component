      ...
      {files.map((file, index) => (
        <div key={index} className="flex items-center justify-between p-3 rounded-md border shadow">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 bg-gray-300 dark:bg-gray-600 rounded flex items-center justify-center p-5">
...
<FileUpload
  uploadMode="single"
  defaultText="Drag and drop or click to upload file"
  otherText="Supported formats: jpg, jpeg, png, webp"
  onFilesUploaded={(files: File | File[] | null) => {
    if (files && !Array.isArray(files)) {
      field.onChange(files);
      // Logic untuk preview atau path file jika diperlukan
    }
  }}
  acceptedFileTypes={{
    "image/jpeg": [".jpg", ".jpeg"],
